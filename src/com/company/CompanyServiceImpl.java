...
//@param Company company, List<Company> companies
        public long getEmployeeCountForCompanyAndChildren(Company company, List<Company> companies) {
            Pair<Long, List<Company>> initPair =
                    this.getChildElementsFromCompany(company, companies, company.getEmployeesCount());
            List<Company> companyToSearch = (List)initPair.getValue();
            Long count = (Long)initPair.getKey();

            for(int i = 0; i < companyToSearch.size(); ++i) {
                Pair<Long, List<Company>> iterationResult =
                        this.getChildElementsFromCompany((Company)companyToSearch.get(i), companies, count);
        ...
        private Pair<Long, List<Company>> getChildElementsFromCompany
                (Company searchItem, List<Company> companies, Long count) {
